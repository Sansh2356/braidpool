...
impl Braid {
    ...
    ///
    /// Uses an explicit worklist instead of recursing through `extend`, so a long chain of
    /// parked orphans cannot overflow the stack (braidpool#532). Orphans waiting on the same
    /// parent are connected in hash order so every node assigns them the same braid indices.
    /// Returns the promoted beads in the order they were inserted into the braid.
